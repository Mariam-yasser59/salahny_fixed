...
class _DiagHistoryScreenState extends State<DiagHistoryScreen> {
  ...
  List<DiagnosticReport> get _filtered {
    ...
    return DiagnosticReport.mockHistory
        .where((r) => r.riskLevel == _filter.toLowerCase())
        .toList();
  }
      ...
      children: [
        ...
        Expanded(
          child: _filtered.isEmpty
              ? const EmptyState(icon: '🔍', title: 'No Records', sub: 'No diagnostic reports match this filter')
              : ListView.separated(
...
class _HistoryTile extends StatelessWidget {
  ...
  IconData get _icon => switch (report.riskLevel) {
    RiskLevel.healthy  => Icons.check_circle_rounded,
    RiskLevel.critical => Icons.error_rounded,
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(report.summary,
                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AC.t1),
                  maxLines: 2, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 5),
              Row(children: [
                const Icon(Icons.calendar_today_rounded, size: 11, color: AC.t3),
                const SizedBox(width: 4),
                Text(report.date, style: const TextStyle(fontSize: 11, color: AC.t3)),
                const SizedBox(width: 10),
                if (report.faultCodes.isNotEmpty) ...[
                  const Icon(Icons.code_rounded, size: 11, color: AC.t3),
                  const SizedBox(width: 3),
                  Text('${report.faultCodes.length} codes', style: const TextStyle(fontSize: 11, color: AC.t3)),
                ],
              ]),
            ]),
          ),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${report.health.toInt()}%', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: _riskColor)),
